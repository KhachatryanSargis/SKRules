...
**iOS/Swift-specific criteria:**

| Criterion | Pass Condition |
|-----------|---------------|
| Sendable/Concurrency | All types used across isolation boundaries are Sendable; @MainActor applied correctly |
| Memory safety | No retain cycles, proper use of weak references where needed |
| Keychain security | Sensitive data stored in Keychain, not UserDefaults or plaintext |
| @MainActor correctness | UI updates happen on MainActor; no blocking operations on main thread |
| Memory leak patterns | No closures that capture self strongly without escape hatch, no unbalanced allocation/deallocation |
...
#### Reviewer B: Optional Dual-Model Review

**Primary mode (default):** Claude performs the review using the code-reviewer agent guidelines.

**Optional dual-model mode:** If external AI CLI tools are available (`codex`, `gemini`), the loop can request a second independent review for higher confidence. To enable:
```bash
export SANTA_DUAL_REVIEW=true  # Requires codex or gemini CLI installed
```

When dual-review is enabled, detect which CLIs are available:
```bash
...
**External Model CLI** (if installed)
```bash
# Use available external-reviewer-model (e.g., secondary-model CLI)
external-reviewer-model -p "$(cat "$PROMPT_FILE")" -C "$(pwd)"
rm -f "$PROMPT_FILE"
```

**Claude-only mode** (when external tools are not found)
When dual-review is enabled but external tools are not found:
1. Log: "External reviewer not found — continuing with Claude-only review"
2. Proceed with single-reviewer mode (no degradation in core functionality)
...
- Reviewer A (Claude Opus) always runs — Claude-only review is the primary, fully-functional mode.
- Dual-model review is OPTIONAL (requires external CLI tools). It adds model diversity but is not required for quality.

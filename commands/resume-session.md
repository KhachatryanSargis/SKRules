...
PROJECT: my-app — Token-Based Authentication

WHAT WE'RE BUILDING:
User authentication with JWT tokens stored in Keychain.
AuthService register/login calls are done. Keychain storage
and AuthCoordinator navigation gating haven't been started yet.

CURRENT STATE:
PASS: Working: 3 items (AuthService.register, JWT decoding, password hashing)
 In Progress: Sources/Auth/TokenManager.swift (decode works, Keychain not yet)
 Not Started: KeychainHelper.swift, AuthCoordinator.swift, LoginView.swift

WHAT NOT TO RETRY:
FAIL: UserDefaults for token — insecure, visible in plain text in sandbox
FAIL: @AppStorage for auth state — wraps UserDefaults (same issue), also caused @Observable update problems

OPEN QUESTIONS / BLOCKERS:
- Should TokenManager be an actor or @MainActor class?

NEXT STEP:
In TokenManager.swift — implement Keychain save/load using SecItemAdd and
SecItemCopyMatching with kSecAttrAccessibleWhenUnlockedThisDeviceOnly.
Then write a unit test with mock KeychainHelperProtocol.

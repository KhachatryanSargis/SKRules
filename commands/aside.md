...
fetchWithRetry() (Sources/Networking/RetryHandler.swift:12) attempts the URLSession
request up to 3 times with exponential backoff (250ms → 500ms → 1s). It only retries
on 5xx responses and network timeouts — 4xx errors are treated as final and not retried.

— Back to task: refactoring the AuthCoordinator in Sources/Coordinators/AuthCoordinator.swift
```
...
No — the shared cache in Sources/Cache/CacheStore.swift:34 is a class with mutable
dictionary accessed from multiple tasks without isolation. This is a data race under
Swift 6 strict concurrency. Consider making CacheStore an actor or protecting access
with a lock/serial queue.

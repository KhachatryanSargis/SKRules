...
9. **UI Framework** — SwiftUI vs UIKit in the affected area
10. **Navigation** — NavigationStack, coordinators, deep links pattern
11. **Data Persistence** — Core Data/SwiftData/UserDefaults/Keychain patterns
12. **Concurrency** — async/await, @MainActor, actor isolation patterns
13. **Security** — Sensitive data storage patterns (consult package CLAUDE.md files for domain-specific rules)
...
|---|---|---|---|
| Naming | `Sources/Services/UserService.swift:1-5` | PascalCase classes/services, camelCase methods | `class UserService` |
| Error | `Sources/Utilities/ErrorHandler.swift:10-25` | Custom Error enum or NSError subclass | `throw UserError.notFound` |
| ... | ... | ... | ... |
...
swift build
```
EXPECT: Zero type errors, build succeeds

### Unit Tests
```bash
# Run tests for affected area
swift test [test-target-name]
```
...
### Core Data Validation (if applicable)
```bash
# Verify data model and migrations
xcodebuild test -scheme YourScheme -configuration Debug
```
EXPECT: Core Data/SwiftData operations work correctly

### UI Validation (if applicable)
```bash
# Run UI tests or manually verify
xcodebuild test -scheme YourSchemeUITests
```
EXPECT: Feature works as designed in simulator/device
...
Consult `rules/architecture.md` and `rules/code-style.md` for iOS-specific patterns (MainActor, memory management, concurrency, Core Data migrations). Do not restate them here — reference the source of truth.
...
Before finalizing, apply the **No Prior Knowledge Test**: a developer unfamiliar with this codebase should be able to implement the feature using ONLY this plan, without searching the codebase or asking questions. If not, add the missing context.

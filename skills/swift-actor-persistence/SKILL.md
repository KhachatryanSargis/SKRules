...
### File Coordination for Shared Containers

If your app uses **App Groups** (shared data with widgets/extensions) or **iCloud document sync**, you must use `NSFileCoordinator` to prevent concurrent file corruption:

```swift
public actor LocalRepository<T: Codable & Identifiable> where T.ID == String {
    // ... (same cache and fileURL properties)

    private func persistToFileCoordinated() throws {
        let data = try JSONEncoder().encode(Array(cache.values))
...
Use `persistToFileCoordinated()` instead of `persistToFile()` when the file is in a shared App Group container or iCloud-synced directory. For single-app, single-process use, the simpler `persistToFile()` is sufficient.
